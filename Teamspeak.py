...
class TeamSpeak:
    ...
    def registerEvent(self, **args):
        ...
        if args.has_key('event') and isinstance(args['event'], str):
            event = args['event']
        elif args.has_key('event') and not isinstance(args['event'],
                str):
            raise Exception('Event is not a string')

        if args.has_key('callback') and hasattr(args['callback'],
                '__call__'):
            callback = args['callback']
        elif args.has_key('callback') and not hasattr(args['callback'],
                '__call__'):
            raise Exception('callback is not a function')

        if args.has_key('threaded') and isinstance(args['threaded'],
                bool):
            threaded = args['threaded']
        elif args.has_key('threaded') and not isinstance(args['threaded'
                ], bool):
            raise Exception('threaded is not a boolean')

        if args.has_key('channel') and isinstance(args['channel'], int):
            cid = args['channel']
        elif args.has_key('channel') and not isinstance(args['channel'
                ], int):
            raise Exception('channel is not a integer')

        if args.has_key('username') and isinstance(args['username'],
                str):
            username = args['username']
        elif args.has_key('username') and not isinstance(args['username'
                ], str):
            raise Exception('username is not a string')

        if args.has_key('password') and isinstance(args['password'],
                str):
            username = args['password']
        elif args.has_key('password') and not isinstance(args['password'
                ], str):
        ...
        if username != None and password != None:
            data = self.decode(self.sendCommand('login '
                               + encode(username) + ' '
                               + encode(password)))
            if data['id'] != '0':
        ...
        if not cid == None:
            ...
            data = self.decode(self.sendCommand('clientmove '
                               + self.encode({'clid': whoami['client_id'
                               ], 'cid': cid})))
            if data['id'] != '0':
                raise Exception('Could not move client: ' + data['msg'])
        command = 'servernotifyregister ' \
            + self.encode({'event': event})
        if not cid == None:
            command = 'servernotifyregister ' \
                + self.encode({'event': event, 'id': cid})
        ...
        while True:
            try:
                ...
                if threaded and callback != None:  # Are we doing thread magic?
                    ...
                    if callback(self, message) == False:
                        return
                else:

                      # Default callback

                    print self.decode(message)['invokername'] + ': ' \
                        + self.decode(message)['msg']
            except NotImplementedError, e:
                raise NotImplementedError('Can\'t thread')  # DDDDDDDouble exception!
            except:
                self.connect()
                if username != None:
                    data = self.decode(self.sendCommand('login '
                ...
                if not cid == None:
                    ...
                    data = self.decode(self.sendCommand('clientmove '
                            ...
                            'cid': cid})))
                    if data['id'] != '0':
                        raise Exception('Could not move client: '
                                + data['msg'])
                command = 'servernotifyregister ' \
                    + self.encode({'event': event})
                if not cid == None:
